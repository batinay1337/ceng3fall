...
void write_union(FILE *file, RECORD movies1, RECORD movies2) {
	...
	FILE *movee= fopen("movies_union.txt", "rb");
	RECORD unions= (RECORD)malloc(sizeof(RECORD_t)*200); 
	fread(unions, sizeof(RECORD_t) * uniSize, 1, movee);

	print_all_movies(unions,uniSize);
