...
int hash(int key){
	...
	return (key*key*key+ 2*key*key) % HASH_SIZE;
}
...
void put(HASH_TABLE htable, int key, void *value){
    ...
    else if(htable[index].flag == 0){
    AVL_TREE tree = linked_tree_init();
	tree->root = linked_tree_insert(tree->root,avl_key,htable[index].value);
	avl_key++;
    tree->root = linked_tree_insert(tree->root,avl_key,value);
	avl_key++;
	htable[index].value = tree;
	htable[index].flag = 1;

    }
    else{
    
	((AVL_TREE)(htable[index].value))->root = linked_tree_insert(((AVL_TREE)(htable[index].value))->root,avl_key,value);
	avl_key++;
}
...
{
linked_tree_horizontal_print(((AVL_TREE)(h1[i].value))->root,avl_key);
}
}
